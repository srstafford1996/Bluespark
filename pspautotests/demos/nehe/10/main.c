...
//                                - joystick use added because it's more easy to move with

// Used keys :
// START = exit 
// joystick = move arround
// pad up= tilt up.
// pad down =  tilt down.
...
{	
    ...
    if (!ImageLoad("Data/mud.bmp", image1)) {
	exit(1);
    }        
...
//    usleep(100); @@@
    ...
    switch (key) {    
    case 'a':			/* startbutton */ // kill everything.
...
/* The function called whenever a normal key is pressed. */
// @@@ this function is modified in  order to use PSP keys
...
// @@@ joystick use added because it's more easy to move with
// @@@ ((float)x / 500.0f) added to use analogic move
void joystickMoved (unsigned int buttonMask, int x, int y, int z)
{
	if (abs(x) > 150) // dead zone
	{	
		// look left and right
		yrot -= 1.5f * ((float)x / 500.0f);  // @@@
	}

	if (abs(y) > 150) // dead zone
	{	
		// walk forward and back (bob head)
		xpos += (float)sin(yrot*piover180) * 0.05f * ((float)y / 500.0f); // @@@
		zpos += (float)cos(yrot*piover180) * 0.05f * ((float)y / 500.0f); // @@@	
		if (walkbiasangle <= 1.0f)
			walkbiasangle = 359.0f;	
		else 
			walkbiasangle-= 10.0 * ((float)y / 500.0f);  // @@@
...
//    glutFullScreen(); @@@
	...
	glutJoystickFunc(&joystickMoved, 0); // @@@ joystick use added
