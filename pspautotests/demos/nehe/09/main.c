//From cygwin version of Nehe tutorial lesson 9
...
//    usleep(100); @@@
    ...
    switch (key) {    
    case 'o':			/* round */ // switch the twinkling.
...
void triggerHandle (int button, int state, int x, int y)
{
	if (button == GLUT_LEFT_BUTTON) {  // left trigger...
		if (state == GLUT_DOWN) {  // ...is pressed
			zoom-=0.02f; // move into the distance.
		}
